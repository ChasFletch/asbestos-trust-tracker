...
export default function NewsDetail() {
  ...
  return <article className="container py-8 max-w-3xl">
    ...
    <header className="mb-8 pb-6 border-b border-border/40">
      <div className="flex flex-wrap items-center gap-2 mb-3"><span className="text-xs font-mono px-1.5 py-0.5 rounded bg-primary/10 text-primary/80 border border-primary/20">Filed quarterly report</span><span className="text-xs text-muted-foreground flex items-center gap-1"><Calendar size={10} />{new Date(`${brief.date}T12:00:00Z`).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric", timeZone: "UTC" })}</span></div>
      ...
      <a href={brief.sourceUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-primary hover:underline mt-4"><ExternalLink size={12} /> View filed Manville Q2 source PDF</a>
    </header>
    ...
    <ArticleAuthorshipReviewBox author={RESEARCH_DESK} legalReviewers={DESIGNATED_LEGAL_REVIEWERS.map((person) => ({ person }))} publishedAt={brief.date} materiallyUpdatedAt={brief.date} sourceCutoffAt="2026-06-30" />
