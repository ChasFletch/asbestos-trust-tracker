/**
 * ⚠️ DEPRECATED — DO NOT RE-RUN WITHOUT REVIEW
 *
 * This seed script is RETAINED FOR REFERENCE ONLY.
 * Since 2026-07-28, trust-figures.json is the single source of truth.
 * All public routes now read financials from JSON, not the DB.
 *
 * Running this script will OVERWRITE the DB with potentially stale data.
 * If you need to bootstrap a fresh DB, update the values below to match
 * client/src/data/trust-figures.json first.
 *
 * Known wrong values in this file (vs JSON):
 *   - Aggregate remainingLow: 17041946126 → 17042880106
 *   - NARCO netAssets: 1325000000 → 1260412792
 *   - OC/Fibreboard netAssets: 1200000000 → 1050000000
 *   - Armstrong netAssets: 450000000 → 225000000
 *   - Federal-Mogul netAssets: 800000000 → 600000000
 *   - USG netAssets: 350000000 → 225000000
 *   - CE Trust netAssets: 600000000 → 450000000
 */
...
// ── Load trust-figures.json as the canonical source ───────────────────────────
const FIGURES_PATH = resolve(process.cwd(), "client/src/data/trust-figures.json");
let canonical = null;
try {
  canonical = JSON.parse(readFileSync(FIGURES_PATH, "utf-8"));
  console.log(`✓ Loaded canonical trust figures (${canonical.trusts?.length ?? 0} trusts) from ${FIGURES_PATH}`);
} catch (e) {
  console.warn("⚠️ Could not load trust-figures.json — falling back to hardcoded seed values.", e);
}
...
await conn.execute(`INSERT INTO aggregate_snapshots
  ...
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  [
    agg.remainingAssetsPoint ?? 17042880106,
    agg.remainingAssetsHigh ?? 22500000000,
    `$${(agg.remainingAssetsPoint ?? 17042880106).toLocaleString()} documented floor`,
    agg.cumulativePayoutsPoint ?? 24000000000,
    `~$${((agg.cumulativePayoutsPoint ?? 24000000000) / 1e9).toFixed(0)}B since 1988 (est.)`,
    agg.activeTrustsEstimated ?? 60,
    ...
    `Mixed 2021–${(canonical?.asOf ?? "2026").substring(0, 4)} as-of dates across trusts; see trust-figures.json for per-trust sources.`,
    1
  ]
);
console.log("✓ aggregate_snapshots seeded from trust-figures.json");

// ── Trusts ────────────────────────────────────────────────────────────────────
// Helper: overlay canonical JSON values onto seed records
function jsonField(trustName, field) {
  const jt = canonical?.trusts?.find((t) => t.name === trustName || t.shortName === trustName);
  return jt ? jt[field] : undefined;
}
...
const trusts = [
  ...
  { id:"wrg", name:"WRG Asbestos PI Trust", shortName:"W.R. Grace", company:"W.R. Grace & Co.", established:2014, administrator:"DCPF", court:"Bankr. D. Del.", docket:"01-01139", website:"https://www.wrgasbestospitrust.com", paymentPct:100.0, paymentPctEffective:"2014-01-01", netAssets:jsonField("WRG Asbestos PI Trust","netAssets") ?? 1840000000, netAssetsAsOf:jsonField("WRG Asbestos PI Trust","assetsAsOf") ?? "2024-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary: trust website; primary doc (Bankr. D. Del. 01-01139 Doc 33338) not yet in RECAP archive", cumulativePaid:2500000000, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"FY2025 annual report filed 2026-04-29 (Doc 33347). 100% payment percentage." },
  { id:"pcc", name:"Pittsburgh Corning Corporation Asbestos PI Settlement Trust", shortName:"Pittsburgh Corning", company:"Pittsburgh Corning Corporation", established:2016, administrator:"DCPF", court:"Bankr. W.D. Pa.", docket:"00-22876", website:"https://www.pccasbestostrust.com", paymentPct:19.0, paymentPctEffective:"2024-11-07", netAssets:jsonField("Pittsburgh Corning Corporation Asbestos PI Settlement Trust","netAssets") ?? null, netAssetsAsOf:jsonField("Pittsburgh Corning Corporation Asbestos PI Settlement Trust","assetsAsOf") ?? null, netAssetsSource:"c", netAssetsCitation:"FY2024 annual report (Bankr. W.D. Pa. Doc 10964) not in RECAP archive; PACER pull pending", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"down", notes:"Cut from 24.5% to 19% effective Nov 7, 2024. Case reopened Aug 2024 to appoint successor FCR Edwin J. Harron." },
  { id:"celotex", name:"Celotex Asbestos Settlement Trust", shortName:"Celotex", company:"Celotex Corporation", established:1996, administrator:"DCPF", court:"Bankr. M.D. Fla.", docket:"8:90-bk-10016", website:"https://www.celotextrust.com", paymentPct:7.0, paymentPctEffective:"2023-06-23", netAssets:jsonField("Celotex Asbestos Settlement Trust","netAssets") ?? null, netAssetsAsOf:jsonField("Celotex Asbestos Settlement Trust","assetsAsOf") ?? null, netAssetsSource:"c", netAssetsCitation:"FY2024 annual report not yet retrieved; PACER pull pending", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"Claimant-elected deferral option available (not a trust-initiated suspension). 7% rate unchanged since June 23, 2023." },
  { id:"bw", name:"B&W Asbestos Settlement Trust", shortName:"Babcock & Wilcox", company:"Babcock & Wilcox Enterprises", established:2006, administrator:"DCPF", court:"Bankr. E.D. La.", docket:"2:00-bk-10992", website:"https://www.bwasbestostrust.com", paymentPct:4.3, paymentPctEffective:"2026-06-30", netAssets:jsonField("B&W Asbestos Settlement Trust","netAssets") ?? null, netAssetsAsOf:jsonField("B&W Asbestos Settlement Trust","assetsAsOf") ?? null, netAssetsSource:"c", netAssetsCitation:"FY2024 annual report not yet retrieved; PACER pull pending", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"down", notes:"Cut from 4.7% to 4.3% effective June 30, 2026. Most recent cut in the system." },
  { id:"narco", name:"NARCO Asbestos Trust", shortName:"NARCO", company:"North American Refractories Company", established:2013, administrator:"DCPF", court:"Bankr. W.D. Pa.", docket:"02-20198", website:"https://www.narcotrust.com", paymentPct:100.0, paymentPctEffective:"2013-01-01", netAssets:jsonField("NARCO Asbestos Trust","netAssets") ?? 1325000000, netAssetsAsOf:jsonField("NARCO Asbestos Trust","assetsAsOf") ?? "2023-12-31", netAssetsSource:"b", netAssetsCitation:"Honeywell $1.325B contribution January 2023; secondary sources", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"100% payment percentage. Honeywell contributed $1.325B in Jan 2023. Many directories show stale pre-2023 balance of $280.7M — incorrect." },
  { id:"oc_fibreboard", name:"Owens Corning/Fibreboard Asbestos Personal Injury Trust", shortName:"OC/Fibreboard", company:"Owens Corning / Fibreboard Corporation", established:2006, administrator:"DCPF", court:"Bankr. N.D. Ohio", docket:"00-03837", website:"https://www.ocfasbestostrust.com", paymentPct:6.0, paymentPctEffective:"2022-01-01", netAssets:jsonField("Owens Corning/Fibreboard Asbestos Personal Injury Trust","netAssets") ?? 1200000000, netAssetsAsOf:jsonField("Owens Corning/Fibreboard Asbestos Personal Injury Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary estimate; primary annual report not retrieved", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"One of the largest trusts by claim volume." },
  { id:"armstrong", name:"Armstrong World Industries Asbestos Personal Injury Settlement Trust", shortName:"Armstrong", company:"Armstrong World Industries", established:2006, administrator:"DCPF", court:"Bankr. D. Del.", docket:"00-04469", website:"https://www.armstrongworldasbestostrust.com", paymentPct:100.0, paymentPctEffective:"2006-01-01", netAssets:jsonField("Armstrong World Industries Asbestos Personal Injury Settlement Trust","netAssets") ?? 450000000, netAssetsAsOf:jsonField("Armstrong World Industries Asbestos Personal Injury Settlement Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary estimate", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"100% payment percentage." },
  { id:"federal_mogul", name:"Federal-Mogul Asbestos Personal Injury Trust", shortName:"Federal-Mogul", company:"Federal-Mogul Corporation", established:2007, administrator:"DCPF", court:"Bankr. D. Del.", docket:"01-10578", website:"https://www.federalmogulasbestostrust.com", paymentPct:12.0, paymentPctEffective:"2020-01-01", netAssets:jsonField("Federal-Mogul Asbestos Personal Injury Trust","netAssets") ?? 800000000, netAssetsAsOf:jsonField("Federal-Mogul Asbestos Personal Injury Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary estimate", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"" },
  { id:"usg", name:"USG Asbestos Settlement Trust", shortName:"USG", company:"USG Corporation", established:2006, administrator:"DCPF", court:"Bankr. D. Del.", docket:"01-02094", website:"https://www.usgasbestostrust.com", paymentPct:100.0, paymentPctEffective:"2006-01-01", netAssets:jsonField("USG Asbestos Settlement Trust","netAssets") ?? 350000000, netAssetsAsOf:jsonField("USG Asbestos Settlement Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary estimate", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"100% payment percentage." },
  { id:"ce", name:"Combustion Engineering 524(g) Asbestos PI Trust", shortName:"CE Trust", company:"Combustion Engineering Inc.", established:2006, administrator:"Verus LLC", court:"Bankr. D. Del.", docket:"03-10495", website:"https://www.cetrust.com", paymentPct:35.0, paymentPctEffective:"2020-01-01", netAssets:jsonField("Combustion Engineering 524(g) Asbestos PI Trust","netAssets") ?? 600000000, netAssetsAsOf:jsonField("Combustion Engineering 524(g) Asbestos PI Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary estimate", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"ABB subsidiary. Verus LLC administrator (formerly MFR, acquired Aug 2024)." },
  { id:"asarco", name:"ASARCO LLC Asbestos Claims Settlement Trust", shortName:"ASARCO", company:"ASARCO LLC", established:2009, administrator:"Verus LLC", court:"Bankr. S.D. Tex.", docket:"05-21207", website:"https://www.asarcotrust.com", paymentPct:100.0, paymentPctEffective:"2009-01-01", netAssets:jsonField("ASARCO LLC Asbestos Claims Settlement Trust","netAssets") ?? 280000000, netAssetsAsOf:jsonField("ASARCO LLC Asbestos Claims Settlement Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary estimate", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"100% payment percentage." },
  { id:"kaiser", name:"Kaiser Aluminum & Chemical Asbestos PI Trust", shortName:"Kaiser Aluminum", company:"Kaiser Aluminum & Chemical Corporation", established:2006, administrator:"Verus LLC", court:"Bankr. D. Del.", docket:"02-10429", website:"https://www.kaiserasbestostrust.com", paymentPct:100.0, paymentPctEffective:"2006-01-01", netAssets:jsonField("Kaiser Aluminum & Chemical Asbestos PI Trust","netAssets") ?? 200000000, netAssetsAsOf:jsonField("Kaiser Aluminum & Chemical Asbestos PI Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary estimate", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"100% payment percentage." },
  { id:"garlock", name:"Garlock Sealing Technologies Asbestos Settlement Trust", shortName:"Garlock", company:"Garlock Sealing Technologies LLC", established:2017, administrator:"Verus LLC", court:"Bankr. W.D.N.C.", docket:"10-31607", website:"https://www.garlockasbestossettlementtrust.com", paymentPct:25.0, paymentPctEffective:"2017-01-01", netAssets:jsonField("Garlock Sealing Technologies Asbestos Settlement Trust","netAssets") ?? 480000000, netAssetsAsOf:jsonField("Garlock Sealing Technologies Asbestos Settlement Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary estimate", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"" },
  { id:"quigley", name:"Quigley Company Asbestos PI Trust", shortName:"Quigley", company:"Quigley Company Inc. (Pfizer subsidiary)", established:2013, administrator:"Verus LLC", court:"Bankr. S.D.N.Y.", docket:"04-15739", website:"https://www.quigleyasbestostrust.com", paymentPct:100.0, paymentPctEffective:"2013-01-01", netAssets:jsonField("Quigley Company Asbestos PI Trust","netAssets") ?? 100000000, netAssetsAsOf:jsonField("Quigley Company Asbestos PI Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"c", netAssetsCitation:"Estimate; Pfizer-funded trust", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"Pfizer subsidiary. 100% payment percentage." },
  { id:"gi_holdings", name:"G-I Holdings / GAF Asbestos Settlement Trust", shortName:"G-I Holdings / GAF", company:"G-I Holdings Inc. / GAF Corporation", established:2009, administrator:"Verus LLC", court:"Bankr. D.N.J.", docket:"01-30135", website:null, paymentPct:8.0, paymentPctEffective:"2020-01-01", netAssets:jsonField("G-I Holdings / GAF Asbestos Settlement Trust","netAssets") ?? 298900000, netAssetsAsOf:jsonField("G-I Holdings / GAF Asbestos Settlement Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary: Kimi K3 research citing trust records 2022", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"Largest newly-anchored balance from small trust sweep." },
  { id:"western_asbestos", name:"Western Asbestos Settlement Trust", shortName:"Western Asbestos", company:"Western Asbestos Company / Western MacArthur", established:2004, administrator:"CPF", court:"Bankr. N.D. Cal.", docket:"02-46284", website:"https://www.wastrust.com", paymentPct:100.0, paymentPctEffective:"2004-01-01", netAssets:jsonField("Western Asbestos Settlement Trust","netAssets") ?? 450000000, netAssetsAsOf:jsonField("Western Asbestos Settlement Trust","assetsAsOf") ?? "2022-12-31", netAssetsSource:"b", netAssetsCitation:"Secondary estimate", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"stable", notes:"100% payment percentage." },
  { id:"apg", name:"APG Asbestos Trust", shortName:"APG", company:"APG Industries", established:2010, administrator:"Verus LLC", court:"Bankr. D. Del.", docket:"05-10220", website:null, paymentPct:6.0, paymentPctEffective:"2026-06-01", netAssets:jsonField("APG Asbestos Trust","netAssets") ?? null, netAssetsAsOf:jsonField("APG Asbestos Trust","assetsAsOf") ?? null, netAssetsSource:"c", netAssetsCitation:"No public balance figure available", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"up", notes:"Raised from 5.3% to 6.0% effective June 2026 — one of very few trusts increasing payment percentage." },
  { id:"shook_fletcher", name:"Shook & Fletcher Asbestos Settlement Trust", shortName:"Shook & Fletcher", company:"Shook & Fletcher Insulation Co.", established:2006, administrator:"CRMC", court:"Bankr. N.D. Ala.", docket:"02-08834", website:"https://www.shookfletchertrust.com", paymentPct:58.0, paymentPctEffective:"2025-05-30", netAssets:jsonField("Shook & Fletcher Asbestos Settlement Trust","netAssets") ?? null, netAssetsAsOf:jsonField("Shook & Fletcher Asbestos Settlement Trust","assetsAsOf") ?? null, netAssetsSource:"c", netAssetsCitation:"No post-founding balance figure published anywhere", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"annual", status:"active", direction:"up", notes:"Raised from 50% to 58% effective May 30, 2025. No balance figure publicly available." },
  { id:"delticus", name:"Delticus (Bendix / Honeywell Asbestos Liabilities)", shortName:"Delticus", company:"Honeywell International (Bendix)", established:2025, administrator:"Third Point / Delticus", court:null, docket:null, website:null, paymentPct:null, paymentPctEffective:null, netAssets:jsonField("Delticus (Bendix / Honeywell Asbestos Liabilities)","netAssets") ?? 1680000000, netAssetsAsOf:jsonField("Delticus (Bendix / Honeywell Asbestos Liabilities)","assetsAsOf") ?? "2025-10-01", netAssetsSource:"b", netAssetsCitation:"Honeywell press release Oct 1, 2025: $1.68B cash + insurance assets contributed to Delticus", cumulativePaid:null, cumulativeClaims:null, reportingFrequency:"unknown", status:"active", direction:"stable", notes:"NOT a §524(g) trust. Third Point portfolio company. Honeywell fully indemnified. Manages claim flows comparable to a mid-tier trust." },
];
