...
import { Check, Copy, Code2, Tag } from "lucide-react";
...
function getEmbedCode(variant: string, height: string, ref?: string) {
  const params: string[] = [];
...
export function EmbedCodeModal({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const code = getEmbedCode(variant.id, variant.height, refTag.trim() || undefined);
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="mt-4">
          <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1">
            <Tag size={10} />Tracking tag <span className="font-normal text-muted-foreground/60">(optional)</span>
          </label>
          <input
            type="text"
            value={refTag}
            ...
            className="w-full px-3 py-2 rounded-md border border-border bg-background text-sm text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-primary/30"
          />
          <p className="text-[10px] text-muted-foreground/50 mt-1">
            Identifies your site in our analytics so you can see embed performance. Letters, numbers, dots, and hyphens only.
          </p>
        </div>
        ...
        <div className="mt-4 p-3 bg-muted/30 rounded-lg border border-border/50">
          ...
          <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside leading-relaxed">
            <li>Enter your domain name in the tracking tag field (optional but recommended)</li>
